...
+ (NSString *)keyForAppHostTypeConfig {
    return [NSString stringWithFormat:@"%@_%@", kNAHostTypeConfig, [[NSBundle mainBundle] bundleIdentifier]];
}

+ (NSString *)keyForAppManualHostConfig {
    return [NSString stringWithFormat:@"%@_%@", kNAManualHostConfig, [[NSBundle mainBundle] bundleIdentifier]];
}

+ (NSString *)keyForAppManualWebHostConfig {
    return [NSString stringWithFormat:@"%@_%@", kNAManualWebHostConfig, [[NSBundle mainBundle] bundleIdentifier]];
}

+ (NSString *)keyForAppManualHostForServiceConfig {
    return [NSString stringWithFormat:@"%@_%@", kNAManualHostForServiceConfig, [[NSBundle mainBundle] bundleIdentifier]];
}
...
+ (NSString *)webhostWithType:(NAHostType)type {
    NSString *version = [NSString NA_outerVersion];
    switch ([NAHostConfig sharedInstance].appType) {
    case NAAppTypeDefault:
...
- (BOOL)hostIsExist {
    if ([[NSUserDefaults standardUserDefaults] objectForKey:[NAHostConfig keyForAppHostTypeConfig]]) {
        return TRUE;
    }
    return FALSE;
}
...
- (void)setHostType:(NAHostType)hostType {
    [[NSUserDefaults standardUserDefaults] setObject:@(hostType) forKey:[NAHostConfig keyForAppHostTypeConfig]];
...
- (NAHostType)hostType {
    ...
    return [[[NSUserDefaults standardUserDefaults] objectForKey:[NAHostConfig keyForAppHostTypeConfig]] intValue];
#else
    if (![[NSUserDefaults standardUserDefaults] objectForKey:[NAHostConfig keyForAppHostTypeConfig]]) {
...
- (NSString *)hostForUserLoginService {
    ...
    if ([self hostType] == NAHostTypeOut || [self hostType] == NAHostTypeSimulate) {
...
#pragma mark 手动配置信息
- (NSString *)manualHost {
    return [[NSUserDefaults standardUserDefaults] objectForKey:[NAHostConfig keyForAppManualHostConfig]];
}

- (void)setManualHost:(NSString *)manualHost {
    if (manualHost) {
        [[NSUserDefaults standardUserDefaults] setObject:manualHost forKey:[NAHostConfig keyForAppManualHostConfig]];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}

- (NSString *)manualWebAppHost {
    return [[NSUserDefaults standardUserDefaults] objectForKey:[NAHostConfig keyForAppManualWebHostConfig]];
}

- (void)setManualWebAppHost:(NSString *)manualWebAppHost {
    if (manualWebAppHost) {
        [[NSUserDefaults standardUserDefaults] setObject:manualWebAppHost forKey:[NAHostConfig keyForAppManualWebHostConfig]];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}

- (NSString *)manualHostForService {
    return [[NSUserDefaults standardUserDefaults] objectForKey:[NAHostConfig keyForAppManualHostForServiceConfig]];
}

- (void)setManualHostForService:(NSString *)manualHost {
    if (manualHost) {
        [[NSUserDefaults standardUserDefaults] setObject:manualHost forKey:[NAHostConfig keyForAppManualHostForServiceConfig]];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}
