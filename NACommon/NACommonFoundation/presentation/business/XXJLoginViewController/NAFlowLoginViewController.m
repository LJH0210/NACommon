...
#import "NAFlowLoginViewController.h"
#import <BlocksKit/BlocksKit+UIKit.h>
#import "NAHostConfig.h"
#import "UITextField+Extension.h"
#import "NSString+NAFoundation.h"
#import "NAProgress.h"
#import "NAFlowVerifyViewController.h"
...
@interface NAFlowLoginViewController () <NAFlowVerifyDelegate> {
...
@implementation NAFlowLoginViewController

- (void)getVervifyCodeHandler {
    [NAProgress showSuccessWithStatus:@"验证码已发送，请注意查收"];
    NAFlowVerifyViewController *viewController = [[NAFlowVerifyViewController alloc] init];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"填写手机号";
    self.view.backgroundColor = [UIColor NA_colorWithHex:0xE8E8E8];
    ...
    btnBackground.frame = CGRectMake(0, 0, kNADeviceWidth, kNADeviceHeight - 44);
...
#else
    [[NAHostConfig sharedInstance] setHostType:NAHostTypeOut];
#endif
}
...
- (void)actionRight {
    ...
    [bk bk_addButtonWithTitle:hosts[1]
                      handler:^{
                          [[NAHostConfig sharedInstance] setHostType:NAHostTypeSimulate];
                          [self manualConfigFinished];
                      }];
    [bk bk_addButtonWithTitle:hosts[2]
                      handler:^{
                          [[NAHostConfig sharedInstance] setHostType:NAHostTypeIn];
                          [self manualConfigFinished];
                      }];
    [bk bk_addButtonWithTitle:hosts[3]
                      handler:^{
                          NAHostManualConfigVC *manualViewController = [[NAHostManualConfigVC alloc] init];
...
- (void)manualConfigFinished {
    NSString *desc = [NAHostConfig hostDescription];
    [self setGetCodeBtnColor:_btnGetCode.enabled];
...
- (void)actionGetVerify {
    ...
    if ([_mobile length] == 0) {
        [NAProgress showErrorWithStatus:@"请输入号码"];
        return;
    }
    _mobile = [_mobile stringByReplacingOccurrencesOfString:@" " withString:@""];
    [self configHost];

    if (![_mobile NA_isValidateMobile]) {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([textField isEqual:_phoneField] && range.length == 0 && string.length == 1) {
        ...
        NSRange selectedRange = [textField NA_selectedRange];
        ...
        [textField NA_setSelectedRange:selectedRange];
