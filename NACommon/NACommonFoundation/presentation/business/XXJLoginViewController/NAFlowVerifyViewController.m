...
@interface NAFlowVerifyViewController () {
    ...
    UIView *_contentView;
    NAInvalidAfterClickedButton *btnResendCode;
}

@end

@implementation NAFlowVerifyViewController
...
- (void)viewDidLoad {
    ...
    _labelTopRight = [[UILabel alloc] initWithFrame:CGRectMake(104, 75, _contentView.frame.size.width - 120, 21)];
    ...
    _labelError = [[UILabel alloc] initWithFrame:CGRectMake(10, 75, _contentView.frame.size.width - 20, 21)];
    ...
    btnResendCode = [NAInvalidAfterClickedButton buttonWithType:UIButtonTypeCustom];
    btnResendCode.frame = CGRectMake(60, 190, _contentView.frame.size.width - 120, 31);
...
- (void)resendCode:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(resendCode)]) {
        ...
        NAInvalidAfterClickedButton *btn = (NAInvalidAfterClickedButton *)sender;
        [btn startTimer];
    }
}
