...
- (void)setDefaults {
    ...
    self.textHighlightedColor = self.textColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    UIRectFill([self frameForSelection]);
    ...
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
        ...
        if (idx == self.selectedIndex) {
            [self.textHighlightedColor set];
        } else {
            [self.textColor set];
        }

#if __IPHONE_OS_VERSION_MIN_REQUIRED < 60000
        [titleString drawInRect:rect withFont:self.font lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
#else
        [titleString drawInRect:rect withFont:self.font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
#endif
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionIndicatorMode == HMSelectionIndicatorResizesToStringWidth) {
        ...
        return CGRectMake(self.segmentWidth * self.selectedIndex, self.height - self.selectionIndicatorHeight, self.segmentWidth,
...
- (CGRect)frameForSelection {
    ...
    if (self.selectionIndicatorMode == HMSelectionIndicatorResizesToStringWidth) {
        ...
        CGFloat x = ((widthTillEndOfSelectedIndex - widthTillBeforeSelectedIndex) / 2) + (widthTillBeforeSelectedIndex - stringWidth / 2);
        return CGRectMake(x, 0, stringWidth, self.bounds.size.height);
    } else {
        return CGRectMake(self.segmentWidth * self.selectedIndex, 0, self.segmentWidth, self.bounds.size.height);
    }
}
    ...
    if (CGRectIsEmpty(self.frame)) {
        ...
        for (NSString *titleString in self.sectionTitles) {
            ...
            if ([UIDevice currentDevice].systemVersion.floatValue >= 7) {
                stringWidth =
                    [titleString sizeWithAttributes:@{NSFontAttributeName : self.font}].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            } else {
