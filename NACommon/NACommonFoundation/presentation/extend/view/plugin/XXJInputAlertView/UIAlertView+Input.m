...
+ (UIAlertView *)
alertViewStylePlainTextInputWithTitle:(NSString *)title
  ...
  return
      [UIAlertView alertViewStylePlainTextInputWithTitle:title
                                                 message:message
...
+ (void)releaseBlock {
  ...
  _dismissBlock = ^(long btnidx) {
  };
  _textInputDismissBlock = ^(UITextField *tf, long buttonIndex) {
  };
}
  ...
  if (alertView.alertViewStyle == UIAlertViewStylePlainTextInput) {
    if (buttonIndex == [alertView cancelButtonIndex]) {
      ...
      if (_textInputDismissBlock) {
        _textInputDismissBlock([alertView textFieldAtIndex:0],
                               buttonIndex - 1);  // cancel button is button 0
      }
    }
